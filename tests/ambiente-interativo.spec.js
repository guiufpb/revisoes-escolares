...
test('carrega a tela inicial e registra todas as revisões sem chaves duplicadas', async ({
  ...
  expect(registro).toHaveLength(44);
  expect(new Set(registro.map((item) => item.id)).size).toBe(44);
  expect(new Set(registro.map((item) => item.chaveArmazenamento)).size).toBe(44);
